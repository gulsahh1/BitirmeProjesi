...
const Login = (props) => {
  ...
  const loginHandle = () => {
    ...
    if ( email.length == 0 || password.length == 0 ) {
      setIsLoading(false);
        Alert.alert('Hata!', 'Email veya şifre boş olamaz.', [
            {text: 'Tamam'}
        ]);
        return;
    }
      ...
      .catch((error) => {
        ...
        if ( error.code === 'auth/user-not-found' || error.code === 'auth/ınvalıd-emaıl' || error.code === 'auth/wrong-password') {
           setIsLoading(false);
            Alert.alert('Geçersiz Kullanıcı!', 'Email veya şifre yanlış.', [
                {text: 'Tamam'}
            ]);
        }
        return;
      ...
      <View style={{height: windowHeight/2, paddingHorizontal: 10}}>
        <View style={{flex:1, backgroundColor: 'white',paddingHorizontal: 10, borderRadius: 10}}>
          <View style={{marginTop: 30}}>
            ...
            />
            <TouchableOpacity activeOpacity={0.3} onPress={() => props.navigation.navigate("ForgotPassword")}>
              <Text style={{textAlign: 'center', fontSize: 15, fontWeight: 'bold', color:'gray', marginTop: 10}}>Şifreni mi unuttun?</Text>
            </TouchableOpacity>
          </View>
