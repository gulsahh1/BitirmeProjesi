...
const MyInstFollowings = (props) => {
  const dispatch = useDispatch();
  const userData = useSelector(state => state.userData);
  const userInstructorFollowings = useSelector(state => state.userInstructorFollowings);
  ...
  return(
    ...
    <View style={{height: 60, flexDirection: 'row'}}>
      <View style={{justifyContent:'center', flex:1}}>
        ...
        <View style={{flex:3, justifyContent:'center'}}>
          <Text style={{fontSize: 24, fontWeight: 'bold'}}>Takip Ettiği Eğitmenler</Text>
        </View>
    ...
    <ScrollView>
    { loading ?
    (  userInstructorFollowings.map((l, i) => (
...
export default MyInstFollowings;
